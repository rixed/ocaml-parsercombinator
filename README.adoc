...
"Several combinators bound together" >:: (
  fun _ctx ->
    let digit = cond "digit" (fun n ->
      Char.code n >= Char.code '0' && Char.code n <= Char.code '9') '1' in
    let p = digit >>= (fun c ->
...
let range a b msg corr rest =
  cond msg (fun c -> c >= a && c <= b) a corr rest

let decimal_digit corr rest =
  range '0' '9' "digit" corr rest

let non_zero_decimal_digit corr rest =
  range '1' '9' "non-zero digit" corr rest

let underscore corr rest =
  item '_' corr rest
...
let unsigned_decimal_number corr rest =
  let digits corr rest = several ~sep:nosep decimal_digit corr rest in
...
let signed neg p corr rest =
  (p|||
   item ~what:"sign" '+' -+ p |||
   (item ~what:"sign" '-' -+ p >>: neg)) corr rest

let decimal_number corr rest =
  signed Int.neg unsigned_decimal_number corr rest
----
...
[source,ml]
----
let non_decimal_integer base prefix digit corr rest =
  let digits corr rest = several ~sep:nosep digit corr rest in
  (prefix -+ repeat ~sep:underscore digits >>:
     List.fold_left (fun c digits ->
       List.fold_left (fun c digit ->
         c * base + num_of_char digit) c digits) 0) corr rest

let octal_digit corr rest =
  range '0' '7' "octal digit" corr rest

let octal_number corr rest =
  (non_decimal_integer 8 (item ~what:"0" '0') octal_digit |>
   signed Int.neg) corr rest

let hexadecimal_digit corr rest =
...
let hexadecimal_number corr rest =
  let prefix = non_decimal_integer_prefix 'x' in
  (non_decimal_integer 16 prefix hexadecimal_digit |>
   signed Int.neg) corr rest

let binary_digit corr rest =
  range '0' '1' "bit" corr rest

let binary_number corr rest =
...
[source,ml]
----
let integer corr rest =
  (decimal_number |||
   octal_number |||
   hexadecimal_number |||
   binary_number) corr rest
----
...
[source,ml]
----
let fractional_part inv_base digit corr rest =
  let digits corr rest = several ~sep:nosep digit corr rest in
...
let unsigned_decimal_fractional corr rest =
  let dot corr rest = item ~what:"fractional dot" '.' corr rest in
...
let decimal_fractional corr rest =
  signed Float.neg unsigned_decimal_fractional corr rest

let decimal_scientific corr rest =
  ((decimal_fractional |||
...
let fractional corr rest =
  (decimal_fractional |||
   decimal_scientific) corr rest
----
  ...
  fun _ctx ->
    ...
    List.iter (fun (input, output) ->
      assert_same_results Float.print
        [output, no_corr, []]
        ((fractional +- eof "char") no_corr (input_of_string input)))
) ;
----
