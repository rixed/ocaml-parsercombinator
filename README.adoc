...
Notice that when `nay` succeeds, ie when `p` fails, the stream of input tokens
`s` is _not_ advanced (it does not consume any input).  Indeed, we do not know
of how many tokens to advance it (since `p` could operate on several).
...
Finally, we can introduce a _left_leaning_ version of +oneof+ that would try
+p2+ only if +p1+ fails. This can considerably speed up parsing at places where
...
Finally, we also accept well known strings for NaN and infinity special
