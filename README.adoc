...
parsers but may be a bit of a mouthful for the end user to chew. This function
turn it into a more manageable +result+ type:
...
struct
  ...
  type stream =
    channel * non_empty_stream option (* <3> *)

  let print_stream fmt = function
    | _, None -> Printf.fprintf fmt "EOF"
    | _, Some _s -> Printf.fprintf fmt "TODO"
----
...
----
  let rec take = function
    ...
    | (channel, Some s) as some_s ->
      if s.next < s.block.last then
        ct_return (
          Item ((), nth s.block.buffer s.next),
          (channel, Some { s with next = s.next+1 }))
      else (
        match s.block.next_block with
        | Some block ->
          take (channel, Some { block ; next = block.first })
        | None ->
----
...
[source,ml]
----
          ct_bind (read_buf channel) (function
          | None ->
            ct_return (EndOfStream, some_s)
...
module FileReader : ParsersConfig.CONFIG =
struct
  (* Lwt bindings: *)
  type 'a ct = 'a Lwt.t
