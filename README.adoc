...
// Insted of an error budget, a number of success between two errors?
// So the Correction should keep track of the number of success in addition to
...
----
type stream
val take : stream -> (token ParsersMisc.stream_value * stream) ct
----
...
Before taking a closer look at this new +ParsersCorrections.t+ type that would encode
...
----
open ParsersMisc
type 'c t = ('c stream_value * string) ParsersBoundedSet.t
----
...
[source,ml]
----
type 'b result = 'b * ((position * token) ParsersCorrections.t) * stream
type 'b t = (position * token) ParsersCorrections.t -> stream -> 'b result list ct
----
...
type position
type stream
val take : stream -> ((position * token) ParsersMisc.stream_value * stream) ct
----
...
let fail_or_maybe_not msg x (* <1> *) ?pos_from corr rest =
  ct_bind
    ...
    (fun (postok, rest') ->
      if ParsersBoundedSet.is_full corr then (
...
let correction_at pos msg =
  let corr = ParsersBoundedSet.make max_changes in
  change_at corr pos msg

let no_corr = ParsersBoundedSet.make 0
...
let print_correction print_where fmt (pos, msg) =
  Printf.fprintf fmt "%s at %a"
    msg
    (ParsersMisc.print_stream_value print_where) pos
