...
Perl has +eval+ on strings. Therefore we do not want to stop parsing as soon as
we reach a string literal if it is (or could be) passed to the +eval+ function.

=== Parsing with gaps

For the same reason, we want to be able to parse as much as possible of a code
string even if we are missing some parts of it.
...
seems to require the same expensive error detection techniques than to detect
user typos.

Indeed, the only way to detect the best attempt is: had the parsing of this bit
behaved differently then the whole outcome would have been much better.  In
other words, had this parsing step accepted this input as valid, we would
eventually have parsed the whole input.
...
possible shapes a term can have.  Unfortunately we can not follow too closely
perl own parser because of its heavy reliance on left recursion, which a
combinatory parser can't do. Instead, we will have to _force_ _progress_ by
...
let rec list_item corr =
  (((term >>: List.singleton) |||
   ...
   opt_whitespace >>:
   function [Bareword w], true -> [Scalar PerlImm.(String (NoInterpString w))]
          | x, _ -> x) corr

and term = string "todo"
----
