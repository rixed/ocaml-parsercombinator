...
----
type stream
val take : stream -> (token Misc.stream_value * stream) ct
----

With the +stream_value+ type being like an +option+ type with more specific
...
----
type 'b parser =
  Corrections.t -> stream -> ('b * Corrections.t * stream) list ct
----

Before taking a closer look at this new +Corrections.t+ type that would encode
the corrections we must question the usage of a list of results as the return
type.  Firstly, a list is over-specified since the order of the possible
...
signal failure seems not enough if we want to explore the artificial _failure_
of parsers (because we need to store that correction somewhere).
...
success of the +check+ parser. Hence, we turned exploring failure with
...
----
open Misc
type 'c t = ('c stream_value * string) BoundedSet.t
----
...
let change_at corr postok (msg : string) =
  BoundedSet.add corr (postok, msg)
----
...
definitive parser type, that we broke down in two parts for simplicity:

.final parser type
[source,ml]
----
type 'b result = 'b * ((position * token) Corrections.t) * stream
type 'b t = (position * token) Corrections.t -> stream -> 'b result list ct
----
...
type position
type stream
val take : stream -> ((position * token) Misc.stream_value * stream) ct
----
...
let fail_or_maybe_not msg x (* <1> *) ?pos_from corr rest =
  ct_bind
    ...
    (fun (postok, rest') ->
      if BoundedSet.is_full corr then (
        ...
        (* Here we replace postok with x ; shall we try inserting x? *)
        ct_return [x, change_at corr postok msg, rest']))
----
...
let assert_same_results ?msg print_output exp actual =
  let result_printer = print_result print_output in
...
"eof suggests truncation" >:: (
  fun _ctx ->
    assert_same_results Unit.print
      [(), correction_at (Item (0, 'g')) "spurious char",
            List.tl rest ]
      (eof "char" corr rest)
) ;
----
...
let cond msg f x corr rest =
  ct_bind (take rest) (function
    | Item (_pos, tok), rest' when f tok ->
      ct_return [ tok, corr, rest' ]
    | _ ->
      fail_or_maybe_not msg x corr rest)
----
...
which expect anything in the given token range (assuming token behavior in face
of an inequality operator makes sense) :
...
"item error exploration" >:: (
  fun _ctx ->
    assert_same_results Char.print
      ['X', correction_at (Item (0, 'g')) "missing item", List.tl rest]
      (item 'X' corr rest)
) ;
----
...
"any: 'a' or 'b' but not 'z'" >:: (
  fun _ctx ->
    ...
    assert_same_results Char.print
      ['a', correction_at (Item (0, 'z')) "missing item", [] ;
       'b', correction_at (Item (0, 'z')) "missing item", []]
...
----
val repeat :
  sep:'d t -> ?min:int -> ?max:int -> 'b t -> ('b list) t
----
...
[source,ml]
----
val several : sep:'d t -> ?max:int -> 'b t -> ('b list) t
val several_greedy : sep:'z t -> ?max:int -> 'b t -> ('b list) t
val times : sep:'z t -> int -> 'b t -> ('b list) t
----
...
let anything corr rest =
  ct_bind (take rest) (function
    ...
    | Item (_pos, tok), rest' -> ct_return [ tok, corr, rest' ])
----
...
With a printer for +BoundedSet.t+ we could also print corrections:
...
let print_stream_value print_value fmt = function
...
let print_correction print_where fmt (pos, msg) =
  Printf.fprintf fmt "%s at %a"
    msg
    (Misc.print_stream_value print_where) pos

let print_corrections print_where fmt corr =
  BoundedSet.print (print_correction print_where) fmt corr
----

With all this we can print results, which are used in unit tests:

.Parser library: printers
[source,ml]
----
let print_result print_output fmt (x, corr, rest) =
  let print_where fmt (pos, tok) =
    Printf.fprintf fmt "%a:%a"
      print_position pos
      print_token tok in
  Printf.fprintf fmt "(output=%a,corrections=%a,rest=%a)"
    print_output x
    (print_corrections print_where) corr
    print_stream rest
----
...
as words, numbers, etc... We will regroup those in a +Usual+ module
...
.LwtConfig.ml: chained buffers of tokens
[source,ml]
----
open Batteries
open Misc
...
.LwtConfig.ml: reading bytes from a file
[source,ml]
----
open Batteries
...
.Positions.ml: pairing an offset with every token
[source,ml]
----
open Batteries
open Misc
...
.Positions.ml: pairing a line number and column number with every characters
[source,ml]
----
