...
to succeed (as a string) completes the +Correction.t+. For the position we
conveniently reuse the stream output as it can already encode both the position
and the special +EOF+ position:
...
[source,ml]
----
let change_at corr postok (msg : string) =
  ParsersBoundedSet.add corr (postok, msg)
----
...
an aggregated value containing the "best" error found (accompanied by some
statistics, while we are at it).
...
We can update the current best error from another stream item and message with
this simple function:

.keeping track of the best error
[source,ml]
----
let update_error pt m = function
...
type 'b t =
  error option -> (position * token) ParsersCorrections.t -> stream ->
...
- +c+ for the +Corrections.t+;
- +s+ for the +stream+;
- +x+ for a +token+;
- +m+ for a message (of type +string+);
...
[source,ml]
----
let return x e c s = ct_return (e, [x, c, s])
----
  ...
  ct_bind
    ...
    (fun (pt, _s') ->
      if ParsersBoundedSet.is_full c then (
        ...
        return x (update_error pt m e) (change_at c pt m) s))
----
...
let nay p e c s =
  ct_bind (p e c s) (function
    | e', [] -> return () e' c s
    | e', _ -> fail_or_maybe_not "not" () e' c s)
----
...
let assert_same_results ?msg print_output (exp_err, exp) actual =
  ct_bind actual (fun (act_err, got) ->
    (* Many times we do not want to guess the errors (for instance
       when the parser actually succeeds): *)
    let act_err =
      if exp_err = None then None else act_err in
    let printer = IO.to_string (print_results print_output) in
    ct_return (
      (* OUnit really should have an assert_same_elements *)
      assert_equal ~printer ?msg
        (exp_err, List.sort compare exp)
        (act_err, List.sort compare got))) |>
  ignore
...
let eof e c s =
  ct_bind (take s) (function
    ...
    | _ ->
      fail_or_maybe_not
        "spurious content at end of input" ()
        ~pos_from:s e c empty_stream (* <1> *))
----
...
"eof fails" >:: (
  fun _ctx ->
    assert_same_results Unit.print
      (Some { where = Item (0, 'g') ;
              what = "spurious content at end of input" }, [])
...
[source,ml]
----
let cond m f x e c s =
  ct_bind (take s) (function
    | Item (_pos, tok), s' when f tok ->
      return tok e c s'
    | _ ->
      fail_or_maybe_not m x e c s)
----
...
[source,ml]
----
let cond_map m f x e c s =
  ct_bind (take s) (function
    | EndOfStream, _ ->
      fail_or_maybe_not m x e c s
    | Item (_pos, tok), s' ->
      (match f tok with
       | Some v -> return v e c s'
       | None   -> fail_or_maybe_not m x e c s))
----
...
"item error exploration" >:: (
  fun _ctx ->
    ...
    assert_same_results Char.print
      (Some { what = "missing item" ;
              where = Item (0, 'g') },
       [ 'X', correction_at (Item (0, 'g')) "missing item",
...
"any: 'a' or 'b' but not 'z'" >:: (
  fun _ctx ->
    ...
    assert_same_results Char.print
      (Some { what = "missing item" ;
              where = Item (0, 'z') },
       ['a', correction_at (Item (0, 'z')) "missing item", z ;
        'b', correction_at (Item (0, 'z')) "missing item", z])
      (a_or_b None corr z)
) ;
----
...
[source,ml]
----
let rec repeat ~sep ?(min=0) ?max ?what p e c s =
  if max = Some 0 then (
    if min = 0 then return [] e c s
    else ct_bind (take s) (fun (pt, _s') ->
      let m = "Too many "^ Option.default "repetitions" what in
      fail (update_error pt m e) c s)
  ) else (
    ...
    | 0 ->
      ...
      (optional ~def:[] (repeat ~sep ~min:1 ?max ?what p)) e c s
    | 1 ->
...
let repeat_greedy ~sep ?min ?max ?what p =
  repeat ~sep ?min ?max ?what p +- check "extraneous" (nay (sep -+ p))
----
...
let rec repeat_greedy ~sep ?min ?max ?what p =
  ...
  | None | Some 0 ->
    ...
    (check "extraneous" (nay p) >>: fun () -> [])
  | min ->
    repeat ~sep ?min ?max ?what p +-
    (check "extraneous" (nay (sep -+ p)) >>:
    fun _ -> [])
----
...
let anything e c s =
  ct_bind (take s) (function
    | EndOfStream, _s' ->
      let e' = update_error EndOfStream "unexpected end of stream" e in
      fail e' c s
    | Item (_pos, tok), s' ->
      return tok e c s')
----
...
val print_result : ('o BatInnerIO.output -> 'b -> unit) ->
                   'o BatInnerIO.output ->
                   'b possible_result -> unit
val print_results : ('o BatInnerIO.output -> 'b -> unit) ->
                    'o BatInnerIO.output ->
...
let non_zero_decimal_digit e =
  range '1' '9' "non-zero digit" e
...
let underscore e = item '_' e

let unsigned_decimal_number e =
  let digits e = several ~sep:none decimal_digit e in
  (non_zero_decimal_digit +-
   ...
   List.fold_left (fun c digits ->
     List.fold_left (fun c digit ->
       c * 10 + num_of_char digit) c digits) 0 ([first]::next)) e
...
let decimal_number e =
  signed Int.neg unsigned_decimal_number e
----
...
let non_decimal_integer base prefix digit =
  let digits e = several ~sep:none digit e in
...
let unsigned_decimal_fractional e =
  let dot e = item ~what:"fractional dot" '.' e in
...
let decimal_fractional e =
  signed Float.neg unsigned_decimal_fractional e

let decimal_scientific e =
  ((decimal_fractional |||
...
let floating_point e =
  (decimal_fractional |||
   decimal_scientific) e
----
...
let number e =
  ((integer        >>: fun x -> Int x) |||
   (floating_point >>: fun x -> Float x)) e
----
...
let whitespace e =
  repeat_greedy ~sep:none ~what:"whitespaces" (blank ||| newline) e

let opt_whitespace e =
  optional_greedy ~def:[] whitespace e
----
...
[source,ml]
----
let lowercase e = range 'a' 'z' "lowercase" e
let uppercase e = range 'A' 'Z' "uppercase" e
let letter e = (lowercase ||| uppercase) e
let alphanum e = (letter ||| decimal_digit) e
----
...
[source,ml]
----
let term e = (term +- any_binary_op ++ term) e
----
...
[source,ml]
----
let term1 e = ((term2 +- low_precedence_op ++ term2) ||| term2) e
let term2 e = ((term3 +- higher_precedence_op ++ term3) ||| term3) e
(* etc... *)
----
...
[source,ml]
----
let rec this_is_ok e =
  (item '{' -+ this_is_ok ++ item '}') e

let rec this_is_infinite_recursion e =
  (this_is_infinite_recursion ++ anything_else) e

let rec this_is_still_infinite_recursion e =
  (check "a check does not consume" some_check ++
   this_is_still_infinite_recursion) e
----
...
"binary_ops_reducer" >:: (
  fun _ctx ->
    let term e = (decimal_digit >>: fun c -> Term c) e in
    let op e = item '+' e in
